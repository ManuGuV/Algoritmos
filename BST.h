...
public:
  ...
  float creaYOrdena(int *arr, int tam)
  {
    ...
    for(int i=0; i<tam; i++)
    {
      //cout << "Entra al for de la inserción, iteración: " << i << endl;
      insertarTail(raiz, arr[i]); // lama a la función recursiva de inserción
    }
  ...
  void insertarTail(Nodo *raiz, int numero)
  {
    ...
    if(raiz == NULL)
    {
      ...
      //cout << "Guardó el dato " << numero << " en el árbol" << endl;
    }else{
